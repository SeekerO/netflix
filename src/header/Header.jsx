...
const Header = () => {
  ...
  return (
    <div className="flex items-center h-fit px-16 py-1 justify-between">
      <div className="flex items-center">
        ...
        <div className="flex gap-x-5  text-md">
          {/* Buttons */}
          {buttons.map((data, index) => (
            <a key={index} className="focus:font-bold cursor-pointer">
              {data.title}
            </a>
          ))}
        </div>
      ...
      <div className="flex gap-x-4 items-center h-full ">
        ...
        <div className="flex gap-1 items-center">
          <div className="h-[33px] w-[33px] bg-white rounded-md flex-shrink-0" />
          <IoMdArrowDropdown />
        </div>
      </div>
    </div>
